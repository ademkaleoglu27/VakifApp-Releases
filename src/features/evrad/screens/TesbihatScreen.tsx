...
const DetailView = ({ prayer, onBack }: { prayer: PrayerItem; onBack: () => void }) => {
    ...
    // ── Audio & Karaoke Sync ──
    const { playTrack, currentTrack, isPlaying, togglePlayPause, position, duration, seekTo } = useAudio();
    const trackId = `tesbihat-${prayer.id}`;
    const isCurrentPrayerPlaying = currentTrack?.id === trackId && isPlaying;

    const tesbihatSegments = React.useMemo(() => {
        if (!data.sections || !duration) return [];
        return audioSyncEngine.calculateTesbihatSegments(data.sections, duration);
    }, [data.sections, duration]);

    const activeSectionId = React.useMemo(() => {
        if (!isCurrentPrayerPlaying || !tesbihatSegments.length) return null;
        return audioSyncEngine.getActiveSegmentId(position, tesbihatSegments);
    }, [isCurrentPrayerPlaying, position, tesbihatSegments]);

    const handlePlayTesbihat = async () => {
        if (currentTrack?.id === trackId) {
            await togglePlayPause();
            return;
        }

        const prayerAudioMap: Record<string, string> = {
            sabah: 'https://github.com/ademkaleoglu27/VakifApp-Releases/releases/download/v1.0-audio/sabah_asir.mp3',
            ogle: 'https://github.com/ademkaleoglu27/VakifApp-Releases/releases/download/v1.0-audio/ogle_tesbihat.mp3',
            ikindi: 'https://github.com/ademkaleoglu27/VakifApp-Releases/releases/download/v1.0-audio/ikindi_tesbihat.mp3',
            aksam: 'https://github.com/ademkaleoglu27/VakifApp-Releases/releases/download/v1.0-audio/aksam_tesbihat.mp3',
            yatsi: 'https://github.com/ademkaleoglu27/VakifApp-Releases/releases/download/v1.0-audio/yatsi_tesbihat.mp3',
        };

        const audioUrl = prayerAudioMap[prayer.id] || 'https://github.com/ademkaleoglu27/VakifApp-Releases/releases/download/v1.0-audio/sabah_asir.mp3';
        await playTrack({
            id: trackId,
            title: `${prayer.title} Tesbihatı`,
            source: { uri: audioUrl },
        });
    };

    const handleSectionTap = async (sectionId: string) => {
        const targetSeek = audioSyncEngine.getSeekMs(sectionId, tesbihatSegments);
        if (currentTrack?.id === trackId) {
            await seekTo(targetSeek);
        } else {
            await handlePlayTesbihat();
            setTimeout(() => {
                seekTo(targetSeek);
            }, 500);
        }
        ...
        <SafeAreaView style={styles.detailContainer} edges={['top']}>
            ...
            {!isLandscape && (
                <View style={styles.header}>
                    ...
                    <TouchableOpacity
                        style={styles.audioHeaderButton}
                        onPress={handlePlayTesbihat}
                        activeOpacity={0.7}
                    >
                        <Ionicons
                            name={isCurrentPrayerPlaying ? 'pause' : 'play'}
                            size={20}
                            color="#FBBF24"
                        />
                    </TouchableOpacity>
            ...
            <ScrollView contentContainerStyle={styles.contentContainer}>
                {data.sections.map((section, index) => {
                    ...
                    const isActive = isCurrentPrayerPlaying && (activeSectionId === sectionId || activeSectionId === section.id);
                    ...
                    return (
                        <TouchableOpacity
                            ...
                            onPress={() => handleSectionTap(sectionId)}
                            activeOpacity={0.85}
                        >
                            {/* Section Header */}
                            {section.title ? (
                                    ...
                                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                                        {isActive && (
                                            <View style={styles.karaokeBadge}>
                                                <Ionicons name="volume-high" size={13} color="#059669" />
