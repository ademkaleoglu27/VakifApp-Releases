            ...
            for (const item of pendingParams) {
                ...
                try {
                    ...
                    if (item.type === 'INSERT_TRANSACTION') {
                        const { error: err } = await supabase.from('transactions').upsert(payload, { onConflict: 'id' });
                        error = err;
                    } else if (item.type === 'INSERT_DECISION') {
                        const { error: err } = await supabase.from('decisions').upsert(payload, { onConflict: 'id' });
                        error = err;
                        ...
                        const { error: err } = await supabase.from('contacts').upsert(payload, { onConflict: 'id' });
                        error = err;
                    } else if (item.type === 'INSERT_CONTACT_READING') {
                        const { error: err } = await supabase.from('contact_readings').upsert(payload, { onConflict: 'id' });
                        error = err;
                    } else if (item.type === 'INSERT_ASSIGNMENT') {
                        const { error: err } = await supabase.from('assignments').upsert(payload, { onConflict: 'id' });
                        error = err;
                        ...
                        const { error: err } = await supabase.from('announcements').upsert(payload, { onConflict: 'id' });
                        error = err;
                    } else if (item.type === 'INSERT_NOTE') {
                        const { error: err } = await supabase.from('risale_notes').upsert(payload, { onConflict: 'id' });
                        error = err;
                    } else if (item.type === 'INSERT_DECISION_LINK') {
                        const { error: err } = await supabase.from('risale_decision_links').upsert(payload, { onConflict: 'id' });
                        error = err;
                    } else if (item.type === 'INSERT_DECISION_ITEM') {
                        const { error: err } = await supabase.from('decision_items').upsert(payload, { onConflict: 'id' });
                        error = err;
