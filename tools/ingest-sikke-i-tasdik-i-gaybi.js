...
// --- ⚙️ CONFIGURATION ---
const WORK_SLUG = 'sikke_i_tasdik_i_gaybi'; // work_id
const WORK_TITLE = 'Sikke-i Tasdik-i Gaybî';
const BOOK_ID = 'risale.sikke_i_tasdik_i_gaybi@diyanet.tr';
const VERSION = 'v1';
const ORDER_INDEX = 8;                      // After Mesnevî-i Nuriye
const JSON_FILENAME = 'sikke-i-tasdiki-gaybi.json'; // Verified filename
// ------------------------
...
function ingestBook() {
    ...
    const regressionCheck = db.prepare(`
        ...
        FROM sections 
        WHERE work_id IN ('sozler','mektubat','lemalar','sualar','asayi_musa','isaratul_icaz','mesnevi_nuriye') 
        GROUP BY work_id
    `).all();
