-- 20260224_fix_leaderboard_union_types.sql
-- Fixes a devastating runtime crash where UNION ALL fails due to implicit type mismatches 
-- between `reading_logs` and `contact_readings` (e.g. integer vs bigint, date vs timestamptz).
-- When this fails, the app UI silently falls back to offline CACHE, causing new readings to "disappear".
...
BEGIN
    ...
    all_readings AS (
        -- 1. App User Readings (Explicit TYPE CASTING for UNION safety!)
        SELECT 
            rl.user_id::UUID,
            NULL::UUID as contact_id,
            rl.pages_read::BIGINT as pages_read,
            rl.date::DATE as date,
            rl.created_at::TIMESTAMPTZ as created_at
        FROM public.reading_logs rl
        WHERE rl.vakif_id = p_vakif_id

        UNION ALL

        -- 2. Manual Contact Readings (Explicit TYPE CASTING for UNION safety!)
        SELECT 
            c.user_id::UUID,
            cr.contact_id::UUID,
            cr.pages_read::BIGINT as pages_read,
            cr.date::DATE as date,
    ...
    raw_stats AS (
        ...
        SELECT 
            ...
            gi.display_name,
            gi.avatar_url,
            COALESCE(SUM(CASE WHEN ar.date >= v_start_date THEN ar.pages_read ELSE 0 END), 0)::BIGINT as total_pages,
            -- Safe MAX comparison using created_at, or fallback to date just in case
            MAX(COALESCE(ar.created_at, ar.date::TIMESTAMPTZ)) as last_reading_date    
    ...
    SELECT
        ...
        RANK() OVER (ORDER BY rs.total_pages DESC, rs.display_name ASC)::BIGINT as rank,
