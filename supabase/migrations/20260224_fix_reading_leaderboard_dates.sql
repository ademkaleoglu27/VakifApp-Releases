-- 20260224_fix_reading_leaderboard_dates.sql
-- Fixes the issue where last_reading_date was returning null in weekly/monthly views
-- if the user hadn't read in that specific period.
...
BEGIN
    ...
    -- (Test amaçlı iptal edildi)
    -- IF NOT EXISTS (
    --     SELECT 1 FROM public.vakif_memberships vm 
    --     WHERE vm.user_id = auth.uid() AND vm.vakif_id = p_vakif_id
    -- ) THEN
    ...
    WITH raw_stats AS (
        SELECT
            ...
            p.avatar_url,
            COALESCE(SUM(CASE WHEN rl.created_at >= v_start_date THEN rl.pages_read ELSE 0 END), 0) as total_pages,
