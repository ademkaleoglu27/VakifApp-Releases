-- =====================================================
-- SYSTEM HARDENING FINAL V2 (Production Ready)
-- =====================================================
-- Contains:
-- 1. Deduplication (CTID based)
-- 2. Unique Constraints
-- 3. Idempotent Triggers (Upsert protection)
-- 4. Normalized Checks (Case/Trim)
-- 5. Security (Search Path)
...
-- Improvements:
--   - Idempotent Profile Insert (UPSERT)
--   - Preserves existing vakif_id if present
--   - Strict Input Normalization
--   - No implicit fallback pollution
...
BEGIN
  ...
  IF v_input_code <> '' THEN
      ...
      IF v_input_code = 'MISAFIR' THEN
         ...
         SELECT id INTO v_vakif_id FROM public.vakiflar WHERE UPPER(code) = 'MISAFIR';
  ...
  -- 4. Profile Upsert (CRITICAL: Idempotency)
  -- Logic: If profile exists, update metadata but DO NOT overwrite vakif_id if it is already set.
  ...
  ON CONFLICT (id) DO UPDATE SET
    ...
    -- If trigger runs again, we might update role if new logic dictates, strictly speaking user asked for role updateable.
    role = EXCLUDED.role,
    -- RED LINE: Never overwrite an existing vakif connection with a new one (or NULL) via this trigger
    vakif_id = COALESCE(public.profiles.vakif_id, EXCLUDED.vakif_id);

  -- 5. Membership Insert
  -- Logic: Only insert if we actually found a target vakif
...
-- Improvements:
--   - Input Sanitization (Trim/Lower/Upper)
...
BEGIN
    ...
    -- Check Role (Strict Platform Admin)
    ...
    IF EXISTS (SELECT 1 FROM public.vakiflar WHERE UPPER(code) = v_clean_code) THEN
    ...
    IF v_clean_email IS NOT NULL THEN
        -- Secure Lookup
        SELECT id INTO v_admin_user_id FROM auth.users WHERE LOWER(email) = v_clean_email;
