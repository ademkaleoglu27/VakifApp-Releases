-- =====================================================
-- SYSTEM HARDENING FINAL V3 (Production Ready)
-- =====================================================
-- What changed (V3):
-- 1. handle_new_user: code lookups now use UPPER(TRIM(code)) for robustness against whitespace.
-- 2. create_tenant_rpc: Strictly enforces 'platform_admin' role check.
...
BEGIN
  ...
  IF v_input_code <> '' THEN
      ...
      IF v_input_code = 'MISAFIR' THEN
         ...
         -- Case-insensitive match for MISAFIR tenant
  ...
  ON CONFLICT (id) DO UPDATE SET
    ...
    role = EXCLUDED.role,
    -- Never overwrite existing vakif_id
