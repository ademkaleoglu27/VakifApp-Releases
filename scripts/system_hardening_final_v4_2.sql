-- =====================================================
-- SYSTEM HARDENING FINAL V4.2 (Production Ready)
-- =====================================================
-- What changed (V4.2):
-- 1. Index 'vakiflar_code_unique' is now PARTIAL (WHERE code IS NOT NULL AND TRIM(code) <> '').
--    This matches the preflight check and prevents empty/null codes from blocking deployment.
-- 2. Added DROP INDEX IF EXISTS to allow upgrading from non-partial index safe within transaction.

BEGIN;

-- =====================================================
-- 0. PREFLIGHT CHECKS (Safe Guard)
-- =====================================================
DO $$
DECLARE
    v_duplicates TEXT;
BEGIN
    -- Check for Normalized Duplicates (Ignoring NULL or Empty strings)
    SELECT string_agg(normalized_code, ', ') INTO v_duplicates
    FROM (
    ...
    IF v_duplicates IS NOT NULL THEN
        RAISE EXCEPTION 'DEPLOYMENT HALTED: Found duplicate normalized codes: [%]. Please resolve manually or enable Preflight B in script.', v_duplicates;
    END IF;
END $$;

-- =====================================================
-- 0.1 PREFLIGHT B (Auto Cleanup - OPTIONAL)
-- =====================================================
/*
-- UNCOMMENT TO ENABLE AUTO CLEANUP
-- Keeps the OLDEST record, deletes newer duplicates
-- Only targets real codes (not null/empty)
DELETE FROM public.vakiflar
WHERE id IN (
    SELECT id
    FROM (
        SELECT id,
               ROW_NUMBER() OVER (partition BY UPPER(TRIM(code)) ORDER BY created_at ASC) as rnum
        FROM public.vakiflar
        WHERE code IS NOT NULL AND TRIM(code) <> ''
    ) t
    WHERE t.rnum > 1
...
-- =====================================================
-- 1. DB-LEVEL UNIQUENESS (Vakiflar)
-- =====================================================
-- Prevent Race Conditions on Code (PARTIAL INDEX)
-- First drop existing if it was non-partial (Safe upgrade)
DROP INDEX IF EXISTS public.vakiflar_code_unique;

CREATE UNIQUE INDEX IF NOT EXISTS vakiflar_code_unique
ON public.vakiflar (UPPER(TRIM(code)))
WHERE code IS NOT NULL AND TRIM(code) <> '';


-- =====================================================
-- 2. CLEANUP DUPLICATES (Robust CTID Method)
-- =====================================================
...
-- =====================================================
-- 4. SECURE & ROBUST TRIGGER (handle_new_user)
-- =====================================================
...
-- =====================================================
-- 5. SECURE RPC (create_tenant_rpc)
-- =====================================================
...
BEGIN
    ...
    -- NOTE: The DB Index we added in step 0 provides a double safety net here.
